const API_URL = "http://localhost:3000/principal";
...
async function editarGasto(id) {
  const novoNome = prompt("Digite o novo nome (ou deixe em branco para manter):");
  if (!novoNome) return; // Cancela se o usuário não digitar nada

  const novoValor = prompt("Digite o novo valor (use ponto para centavos. Ex: 150.50):");
  const novaData = prompt("Digite a nova data (Formato AAAA-MM-DD):");
  const novoTipo = prompt("Digite o novo tipo (Exatamente: 'Fixos' ou 'Variáveis'):");
