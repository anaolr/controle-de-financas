...
// Coloque aqui o link do seu Render!
const BASE_URL = "https://controle-de-financas-9zvz.onrender.com"; // ou "https://meu-gerenciador.onrender.com"
...
async function carregarDados() {
  try {
    const response = await fetch(`${BASE_URL}/principal`, { headers: getHeaderAuth() });
    ...
    fixosBody.innerHTML = ""; variaveisBody.innerHTML = "";
    let somaFixos = 0; let somaVariaveis = 0;

    data.forEach((element) => {
      const tr = document.createElement("tr");
      const valorFormatado = parseFloat(element.valor).toLocaleString("pt-BR", { style: "currency", currency: "BRL" });
      ...
      if (element.tipo === "Fixos") {
        fixosBody.appendChild(tr); somaFixos += parseFloat(element.valor);
      } else {
        variaveisBody.appendChild(tr); somaVariaveis += parseFloat(element.valor);
      }
    });

    document.getElementById("total-fixos").textContent = somaFixos.toLocaleString("pt-BR", { style: "currency", currency: "BRL" });
    document.getElementById("total-variaveis").textContent = somaVariaveis.toLocaleString("pt-BR", { style: "currency", currency: "BRL" });
    document.getElementById("total-geral").textContent = (somaFixos + somaVariaveis).toLocaleString("pt-BR", { style: "currency", currency: "BRL" });
...
async function excluirGasto(id) {
  if (confirm("Tem certeza que deseja excluir?")) {
    await fetch(`${BASE_URL}/principal/${id}`, { method: "DELETE", headers: getHeaderAuth() });
    carregarDados();
  }
}
...
async function editarGasto(id) {
  ...
  await fetch(`${BASE_URL}/principal/${id}`, {
    ...
    body: JSON.stringify({ nome: novoNome, valor: parseFloat(novoValor), data: novaData, tipo: novoTipo }),
  });
  carregarDados();
}
